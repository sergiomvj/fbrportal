import { createDomain, deleteDomain, listDomains, updateDomain } from '@/lib/leads/store';
...
export async function POST(request: Request) {
  ...
  try {
    return Response.json({ domain: createDomain(context, await request.json()) }, { status: 201 });
  } catch (error) {
    return jsonError(error);
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const id = body.id as string;
    if (!id) return Response.json({ code: 'BAD_REQUEST', message: 'id is required.' }, { status: 400 });
    return Response.json({ domain: deleteDomain(context, id) });
  } catch (error) {
    return jsonError(error);
  }
}
