...
- **Global scope**: `C:/Projetos/1FBRPortal/fbrportal/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Projetos/1FBRPortal/fbrportal/.claude/notes/`)
...
2. Glob `C:/Projetos/1FBRPortal/fbrportal/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Projetos/1FBRPortal/fbrportal/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Projetos/1FBRPortal/fbrportal/.claude/notes/` — works in any directory
